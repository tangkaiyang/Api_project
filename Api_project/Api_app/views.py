...
def get_tj_datas(request):
    ...
    tj_datas["contribution"] = {
        "project":  int(len(DB_project_list.objects.filter(creater=userID)) /tj_datas['overview']['project_count'] * 100 ),
...
def get_project_list(request):
    ...
    if keys:
        project_list_data =  list((DB_project_list.objects.filter(name__contains=keys,deleted=False)|DB_project_list.objects.filter(des__contains=keys,deleted=False)).values())[::-1] # select * from 表 where name like %keys% or des like %keys% ;
    else:
...
def import_api_postman(request):
    ...
    for i in item:
        ...
        form_data['payload_method'] = form_data['payload_method'].replace('urlencoded','x-www-form-urlencoded').replace('graphql','GraphQL').replace('formdata','form-data')
